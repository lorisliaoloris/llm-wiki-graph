...
### 刷新机制
- 🔄 **手动刷新按钮**：侧边栏顶部 "🔄 刷新" → 触发 GitHub Actions workflow_dispatch 重建 Pages
- 🔄 **自动感知刷新**：首次加载数据超 1 小时自动刷；离开页面超 5 分钟回来强制刷
- 数据源：ECS API (100.85.182.75:9885) → 拉最新 nodes/lint → 重载页面

### Sources 多层分类
Sources 从 Gbrain 自动分类，支持 3-5 层递归展开：

分类逻辑在 JS 中通过 taxonomy 对象递归匹配  和 。

### Lint 过期分级
过期按时效分 4 档，点开展示具体页面列表：
- 3 天内过期
- 7 天内过期
- 30 天内过期
- 3 个月内过期
数据从 ECS API  获取，含页面 slug/title/updated_at。

### 最近更新面板
侧边栏底部 "🔥 最近更新" — 展示最近 8 条变更的页面，点击跳转 D3 节点。
数据从 ECS API  获取。

### Schema/Log 内容降级
- Schema: 点击 → 右侧内容面板展示  全文（fetch raw.githubusercontent.com）
- Log: 点击 → 右侧内容面板展示  全文；子节点按日期折叠
- 内容面板有 "← 返回图谱" 按钮关闭
